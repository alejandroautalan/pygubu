...
class TKToplevel(BuilderObject):
    ...
    def _code_set_property(self, targetid, pname, value, code_bag):
        if pname in ('geometry', 'overrideredirect', 'title'):
            ...
            super(TKToplevel, self)._code_set_property(targetid, pname, value, code_bag)
...
class TKLabelFrame(BuilderObject):
    ...
    OPTIONS_SPECIFIC = ('background', 'class_', 'height', 'labelanchor', 'width')
    properties = OPTIONS_STANDARD + OPTIONS_SPECIFIC
...
class TKButton(BuilderObject):
    ...
    OPTIONS_SPECIFIC = ('command', 'default', 'height', 'overrelief', 'state', 'width')
...
register_widget(
    'tk.Checkbutton', TKCheckbutton, 'Checkbutton', ('Control & Display', 'tk')
)
...
register_widget(
    'tk.Radiobutton', TKRadiobutton, 'Radiobutton', ('Control & Display', 'tk')
)
...
register_widget('tk.PanedWindow', TKPanedWindow, 'PanedWindow', ('Containers', 'tk'))
...
register_widget('tk.Message', TKMessage, 'Message', ('Control & Display', 'tk', 'ttk'))
...
class TKScrollbar(BuilderObject):
    ...
    OPTIONS_SPECIFIC = ('activerelief', 'command', 'elementborderwidth', 'width')
    properties = OPTIONS_STANDARD + OPTIONS_SPECIFIC
    command_properties = ('command',)


register_widget('tk.Scrollbar', TKScrollbar, 'Scrollbar', ('Control & Display', 'tk'))
...
register_widget('tk.Canvas', TKCanvas, 'Canvas', ('Control & Display', 'tk', 'ttk'))
...
class TKMenuitem(BuilderObject):
    ...
    def _code_set_property(self, targetid, pname, value, code_bag):
        if pname == 'command_id_arg':
            ...
            super(TKMenuitem, self)._code_set_property(targetid, pname, value, code_bag)
    ...
    def _code_connect_command(self, cmd_pname, cmd, cbname):
        if self.itemtype != 'submenu':
            ...
            return super(TKMenuitem, self)._code_connect_command(cmd_pname, cmd, cbname)
...
class TKMenuitemSubmenu(TKMenuitem):
    ...
    properties = tuple(set(OPTIONS_STANDARD + OPTIONS_SPECIFIC + OPTIONS_CUSTOM))
    ...
    def code_realize(self, boparent, code_identifier=None):
        ...
        code_bag, kw_properties, complex_properties = self._code_process_properties(
            menuprop, self.code_identifier()
        )
        for pname in complex_properties:
            lines.extend(code_bag[pname])
        ...
        # creation
        line = "{0} = tk.Menu({1}{2})".format(self.code_identifier(), masterid, mprops)
        lines.append(line)
...
register_widget(
    'tk.Menuitem.Submenu', TKMenuitemSubmenu, 'Menuitem.Submenu', ('Menu', 'tk', 'ttk')
)
...
register_widget(
    'tk.Menuitem.Command', TKMenuitemCommand, 'Menuitem.Command', ('Menu', 'tk', 'ttk')
)
...
register_widget(
    'tk.OptionMenu', TKOptionMenu, 'OptionMenu', ('Control & Display', 'tk', 'ttk')
)
