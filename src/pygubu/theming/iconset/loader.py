import pathlib
import json
import importlib.resources as resources
from contextlib import suppress
from pygubu.theming.iconset.iconset import IconSet
from pygubu.theming.iconset.svg2photo import svg2photo


class IconSetLoader:
    def __init__(self, data_module, iconset_filename):
        self.master = None
        self.data_module = data_module
        self.iconset_filename = iconset_filename
        self.iconset = None
        self._theme = IconSet.THEME_LIGHT
        self.loaded = {}

        with resources.open_binary(data_module, iconset_filename) as cf:
            self.iconset = IconSet(json.load(cf))

    def _check_master(self):
        if self.master is None:
            raise RuntimeError("master is not configured")

    @property
    def theme(self):
        return self._theme

    @theme.setter
    def theme(self, value):
        theme_values = (IconSet.THEME_LIGHT, IconSet.THEME_DARK)
        if value not in theme_values:
            raise ValueError()
        self._theme = value
        if self.master is not None:
            self._reload_images()

    def _reload_images(self):
        self._check_master()
        for key, photo in self.loaded.items():
            photo.tcl_keep()
            self._load_image(self.master, key)

    def _load_image(self, master, image_uid):
        if self.master is None and master is not None:
            self.master = master
        tkimage = None
        if image_uid in self.iconset:
            fn, size, color = self.iconset.icon_props(image_uid, self._theme)
            with resources.open_binary(self.data_module, fn) as fileio:
                tkimage = svg2photo(
                    fileio,
                    fill=color,
                    scaletowidth=size,
                    master=master,
                    tcl_name=image_uid,
                )
                self.loaded[image_uid] = tkimage
        return tkimage

    def __call__(self, master, image_uid):
        return self._load_image(master, image_uid)
